...
# 修改config里的users选项即可
# 格式：{'username':21201712xx , 'password': 'xxxxxxx', 'address': '江西省赣州市章贡区', 'email': '', 'school': '江西理工大学'}}]}
# username填你的学号，password填你的密码，address填你需要定位的签到地址（可以任意填写），school不变
def getYmlConfig():
    config = {'login':{'api': 'http://www.zimo.wiki:8080/wisedu-unified-login-api-v1.0/api/login'},'users': [{'user': {'username': , 'password': '', 'address': '', 'email': '', 'school': '江西理工大学'}}]}
    return dict(config)
...
def getCpdailyApis(user):
    apis = {'login-url': 'https://authserver.jxust.edu.cn/authserver/login?service=https%3A%2F%2Fjxust.cpdaily.com%2Fportal%2Flogin','host': 'jxust.cpdaily.com'}
    return apis
